...
public:
  TwinkleFox(LedPatterns *ledPatterns) : Pattern(ledPatterns) {
...
private:
  ...
  uint8_t SECONDS_PER_PALETTE = 15;
  ...
  void drawTwinkles() {
    ...
    CRGB bg =
        gBackgroundColor; // just use the explicitly defined background color
    ...
    for (int strip = 0; strip < ledPatterns->nLedEntries; strip++) {
      for (int i = 0; i < ledPatterns->NUM_LEDS; i++) {
        ...
        if (deltabright >= 32 || (!bg))
          ...
          color = blend(bg, c, deltabright * 8);
        else
          color = bg;

        ledPatterns->setLed(strip, i, color);
      }
    }
  }
